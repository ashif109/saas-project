...
export default function FacultyDashboardPage() {
  ...
  useEffect(() => {
    const fetchDashboard = async () => {
      try {
        const res = await api.get('/api/dashboard/faculty-summary');
  ...
  const stats = [
    { label: "Today's Classes", value: dashboardData?.stats?.todaysClasses ?? "-", icon: Calendar, color: "text-blue-500", bg: "bg-blue-50" },
    { label: "Pending Assignments", value: dashboardData?.stats?.pendingAssignments ?? "-", icon: FileText, color: "text-amber-500", bg: "bg-amber-50" },
    { label: "Unresolved Doubts", value: dashboardData?.stats?.unresolvedDoubts ?? "-", icon: MessageSquare, color: "text-rose-500", bg: "bg-rose-50" },
    { label: "Attendance Marked", value: dashboardData?.stats?.attendanceMarked ?? "-/-", icon: CheckCircle2, color: "text-emerald-500", bg: "bg-emerald-50" }
  ];
